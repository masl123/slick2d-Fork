...
class OpenGL2Program extends Program {
	...
	public int attachShader(ShaderType shaderType, String source) {
		...
		case VERTEX:
			...
			IntBuffer vertexVariablesBuffer = null; 
			GL20.glGetProgram(programID, GL20.GL_ACTIVE_UNIFORMS, vertexVariablesBuffer);
			int vertexVariables = vertexVariablesBuffer.array().length;
		...
		case FRAGMENT:
			...
			IntBuffer fragmentVariablesBuffer = null;
			GL20.glGetProgram(programID, GL20.GL_ACTIVE_UNIFORMS, fragmentVariablesBuffer);
			int fragmentVariables = fragmentVariablesBuffer.array().length;
